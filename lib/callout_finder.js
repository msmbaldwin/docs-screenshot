      ...
      try {
        const match = await frame.evaluate((t) => {
          ...
          function findFormFieldBounds(el) {
            ...
            const pad = 4;
            // Copy/action buttons at the far right of a form field often have icons
            // whose visual rendering (including hover highlight areas) extends slightly
            // beyond getBoundingClientRect(). Add extra right padding when the rightmost
            // element is a button to prevent the callout line from clipping through it.
            const rightmostIsButton = Array.from(interactiveChildren).some(child => {
              const r = child.getBoundingClientRect();
              const tag = child.tagName ? child.tagName.toLowerCase() : '';
              return (Math.abs(r.right - maxX) < 2) && (tag === 'button' || child.getAttribute('role') === 'button');
            });
            const rightPad = rightmostIsButton ? 12 : pad;

            const unionWidth = (maxX - minX) + pad + rightPad;
          ...
          function getVisualBounds(el) {
            ...
            // Check if target is inside a dropdown/select/combobox control.
            // If so, walk up to the full control (including indicator/chevron).
            let dropdownContainer = null;
            {
              ...
              for (let i = 0; i < 8 && node; i++) {
                ...
                if (
                  ...
                  cls.includes('dropdown') || cls.includes('fxc-dropdown') ||
                  cls.includes('azc-select') || cls.includes('ms-dropdown') ||
                  cls.includes('fxc-combo')
                ) {
            ...
            if (dropdownContainer) {
              const r = dropdownContainer.getBoundingClientRect();
              if (r.width > 0 && r.height > 0 && r.width < window.innerWidth * 0.6) {
                return {
                  x: Math.round(r.x),
                  y: Math.round(r.y),
                  width: Math.round(r.width),
                  height: Math.round(r.height),
                  element: dropdownContainer.tagName,
                  text: dropdownContainer.textContent.trim().substring(0, 50),
                };
              }
            }
            ...
            for (let i = 0; i < 5 && current.parentElement; i++) {
              ...
              const isContainer = (
                ...
                // Azure portal-specific toolbar containers
                cls.includes('azc-toolbar') || cls.includes('fxs-commandbar') ||
                cls.includes('fxs-startboard') || cls.includes('toolbarbutton')
              );
            ...
            // Expand bestRect to include ALL visible direct children of the
            // container (catches icons, SVGs, images that overflow the layout box).
            // This prevents clipping icons next to menu text.
            // Scan two levels deep to catch icons inside wrapper divs (common in
            // Azure portal toolbar buttons: <li><div><img/><div>text</div></div></li>)
            if (bestContainer !== el && bestContainer.children && bestContainer.children.length > 0) {
              let minX = bestRect.x, minY = bestRect.y;
              let maxX = bestRect.x + bestRect.width;
              let maxY = bestRect.y + bestRect.height;
              let expanded = false;

              const scanChildren = (parent, depth) => {
                for (const child of parent.children) {
                  const cr = child.getBoundingClientRect();
                  if (cr.width === 0 || cr.height === 0) continue;
                  // Only include children that overlap vertically with the container
                  if (cr.bottom < bestRect.y - 4 || cr.top > bestRect.y + bestRect.height + 4) continue;
                  if (cr.left < minX) { minX = cr.left; expanded = true; }
                  if (cr.top < minY) { minY = cr.top; expanded = true; }
                  if (cr.right > maxX) { maxX = cr.right; expanded = true; }
                  if (cr.bottom > maxY) { maxY = cr.bottom; expanded = true; }
                  // Recurse one level deeper for wrapper divs
                  if (depth < 2 && child.children && child.children.length > 0) {
                    scanChildren(child, depth + 1);
                  }
                }
              };
              scanChildren(bestContainer, 0);
            ...
            // Vertical centering: ensure the text is vertically centered within
            // the returned rect. If the container has asymmetric internal padding
            // (e.g., more padding-top than padding-bottom), the text would appear
            // off-center in the callout box. Fix by re-centering on the text.
            const textCenterY = (textRect.top + textRect.bottom) / 2;
            const boxCenterY = (bestRect.y + bestRect.y + bestRect.height) / 2;
            if (Math.abs(textCenterY - boxCenterY) > 2) {
              const distTop = textCenterY - bestRect.y;
              const distBottom = (bestRect.y + bestRect.height) - textCenterY;
              const maxDist = Math.max(distTop, distBottom);
              bestRect = {
                x: bestRect.x,
                y: textCenterY - maxDist,
                width: bestRect.width,
                height: maxDist * 2,
              };
            }

            // Never-clip check: ensure the callout rect does not extend beyond
            // the nearest containing popup, panel, or dropdown boundary.
            {
              let panel = bestContainer.parentElement;
              for (let i = 0; i < 10 && panel; i++) {
                const cls = (panel.className || '').toString().toLowerCase();
                const role = panel.getAttribute ? (panel.getAttribute('role') || '') : '';
                const style = getComputedStyle(panel);
                const isPanel = (
                  role === 'dialog' || role === 'menu' || role === 'listbox' ||
                  role === 'alertdialog' || role === 'tooltip' ||
                  cls.includes('popup') || cls.includes('flyout') ||
                  cls.includes('dropdown-popup') || cls.includes('context-menu') ||
                  cls.includes('fxs-blade') || cls.includes('ms-panel') ||
                  (style.overflow === 'hidden' || style.overflow === 'auto' || style.overflowY === 'auto')
                );
                if (isPanel) {
                  const pr = panel.getBoundingClientRect();
                  if (pr.width > 0 && pr.height > 0 && pr.width < window.innerWidth * 0.9) {
                    const margin = 2; // small inset so we don't touch the border
                    const clampedX = Math.max(pr.left + margin, bestRect.x);
                    const clampedY = Math.max(pr.top + margin, bestRect.y);
                    const clampedR = Math.min(pr.right - margin, bestRect.x + bestRect.width);
                    const clampedB = Math.min(pr.bottom - margin, bestRect.y + bestRect.height);
                    if (clampedR > clampedX && clampedB > clampedY) {
                      bestRect = {
                        x: clampedX, y: clampedY,
                        width: clampedR - clampedX, height: clampedB - clampedY,
                      };
                    }
                  }
                  break;
