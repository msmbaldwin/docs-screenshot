...
- **`user_config.py`**: Loads optional per-user defaults from `~/.config/docs-screenshot/config.yaml` (or `.json`). Used to avoid hardcoding `username`, `subscription`, `tenant_display_name`, `repo_root`, or `custom_replacements` in the rest of `lib/`. Run `python lib/user_config.py` to verify your config is being read.
- **`checkpoint.py`**: Persistent per-article progress tracking for Scenario 2 resumability. Stores per-image completion state under `~/.copilot/skills/docs-screenshot/state/<article-hash>.json` (override with `DOCS_SCREENSHOT_STATE_DIR`). Use `Checkpoint.for_article(path)` + `cp.is_complete()` + `cp.mark_done()` to skip already-completed work on retry. Run `python -m lib.checkpoint` to list existing checkpoints. See [`resumability.md`](resumability.md).
